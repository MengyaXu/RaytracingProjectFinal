...
std::string BRICKS_IMAGE_PATH = "/Users/timothymshepard/Cosc4370/Timothy_Shepard_Final_REDFINAL/bricks.png";
std::string TEAPOT_PATH = "/Users/timothymshepard/Cosc4370/Timothy_Shepard_Final_REDFINAL/teapot2.obj";

material* loadBrickMaterial()
{
	int nx, ny, nn;
	unsigned char *tex_data = stbi_load(BRICKS_IMAGE_PATH.c_str(), &nx, &ny, &nn, 0);
...
hitable *lighted_spheres_scene() {
    ...
    list[4] = new yz_plane(40, new diffuse_light(new constant_texture(vec3(40,40,40))));
    //list[4] = new yz_rect(-70, 1000, 0, 3000, 40, new diffuse_light(new constant_texture(vec3(40,40,40))));
    return new hitable_list(list,n);
}

hitable *spheres_scene() {
    int n = 4;
    texture *red_checker = new checker_texture( new constant_texture( vec3(0.2, 0.1, 0.1)), new constant_texture( vec3(0.7, 0.1, 0.1)), 0.50);
    texture *plane_checker = new plane_checker_texture( new constant_texture( vec3(0.5, 0.5, 0.5)), new constant_texture( vec3(0.1, 0.1, 0.1)), -30.0);
    ...
    list[1] = new sphere(vec3(15, 2, 6), 6.0, new texture_metal(red_checker, 0.0));
    list[2] = new sphere(vec3(2, -5, 5.5), 5.5, loadBrickMaterial());
    list[3] = new sphere(vec3(-8, -16, 5), 5.0, new dielectric(1.52));
    return new hitable_list(list,n);
}

hitable *spheres_triangle_scene() {
    int n = 4;
    texture *red_checker = new checker_texture( new constant_texture( vec3(0.2, 0.1, 0.1)), new constant_texture( vec3(0.7, 0.1, 0.1)), 0.50);
    texture *plane_checker = new plane_checker_texture( new constant_texture( vec3(0.5, 0.5, 0.5)), new constant_texture( vec3(0.1, 0.1, 0.1)), -30.0);
...
hitable *final_project_scene() {
    ...
    list[0] = new xy_plane(0.0, new texture_metal(plane_checker, 0.0));
    list[1] = new sphere(vec3(15, 2, 6), 6.0, new texture_metal(red_checker, 0.0));
    list[2] = new sphere(vec3(-8, -16, 5), 5.0, new dielectric(1.52));
...
int main(int argc, char *argv[]) {
    if(argc != 2)
    {
	std::cout << "usage: " << argv[0] << " <ppm_file_name like scene.ppm>\n";
	return 1;
    }
